...
import argparse
import signal
from ardu_utils import ArducamUtils
...
def display(cap, arducam_utils):
    ...
    while True:
        ...
        g = cv2.multiply(g, 0.75)  # G값을 줄여 색감을 조정
        frame = cv2.merge([b, g.astype(np.uint8), r])

        frame = cv2.cvtColor(frame, cv2.COLOR_BGR2YUV_I420)
        #f.write(frame.tobytes())  # BGR을 그대로 write
